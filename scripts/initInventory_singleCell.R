...
######## 
ageInit <- studyArea
ageInit[!is.na(studyArea)] <- simInfo$ageinit
#### exporting raster
...
cls_sp <-  studyArea
cls_sp[!is.na(studyArea)] <- simInfo$coverTypes

ct_AT <- read.csv(paste(sourceDir, "coverTypes_RAT.csv", sep = "/"))
# adding CBM spp names
ct_AT$AIDBSPP <- c(EN = "Black spruce",
                   PG = "Jack pine")[as.character(ct_AT[,"value"])]

ct_AT <- ct_AT[complete.cases(ct_AT),]

cls_fert <- cls_dens <- studyArea

fertVals <- unique(simInfo$siteIndex)
cls_fert[!is.na(studyArea)] <- match(simInfo$siteIndex, fertVals)

densVals <- unique(simInfo$rho100)
cls_dens[!is.na(studyArea)] <- match(simInfo$rho100, densVals)
...
fert_AT <- data.frame(ID = 1:length(fertVals),
                      cls_fert = fertVals,
                      value = fertVals)
dens_AT <- data.frame(ID = 1:length(densVals),
                      cls_dens = densVals,
                      value = densVals)
...
for (s in ct_AT$ID) {
    for (f in fert_AT$ID) {
        for (d in dens_AT$ID) {
            spName <- ct_AT[match(s, ct_AT$ID), "value"]
            AIDBSPP <- ct_AT[match(s, ct_AT$ID), "AIDBSPP"]
