...
cls_dens_tmp <- cls_dens
#regenFail <- studyArea
#regenFail[!is.na(regenFail)] <- simInfo$regenFailure
for (l in unique(simInfo[,"Wild fire"])) {
    
    ## wild fires
    # creating raster
    lName <- paste("fire", l, sep = "_")
    f <- studyArea
    # fetching current density values
    densVals <- values(cls_dens_tmp)
    densVals <- densVals[!is.na(densVals)]
...
## creating raster
last_pass_disturbance_type <- studyArea
last_pass_disturbance_type[!is.na(studyArea)] <- last_pass_disturbance_type_AT[last_pass_disturbance_type_AT$value == "Wild Fire", "ID"]
